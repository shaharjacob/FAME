...
class Quasimodo:
    ...
    def get_pred_and_obj(self, subject, n_largest: int = 0) -> List[Tuple[str]]:
        to_return = []
        df = self.filter_by_subject(subject, n_largest=n_largest)
        for idx, val in df.iterrows():
            to_return.append((str(val['predicate']).replace('_', ' '), str(val['object'])))
        return to_return
    

    def filter_by_subject(self, subject: str, soft: bool = False, df: DataFrame = DataFrame(), n_largest: int = 0) -> DataFrame:
...
if __name__ == '__main__':
    quasimodo = Quasimodo(path='quasimodo_0.5.tsv', score_threshold=0.9)
    res = quasimodo.get_pred_and_obj('horse', 10)
    print(res)
